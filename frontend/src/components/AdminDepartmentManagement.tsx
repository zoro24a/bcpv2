  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!deptForm.name) {
      toast({
        ...
        description: "Please provide the formal department identity.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      await departmentService.create({
        name: deptForm.name,
        established_year: deptForm.established_year ? parseInt(deptForm.established_year) : undefined
      });
      ...
      setDeptForm({ name: '', established_year: '' });
      fetchDepartments();
    } catch (error) {
              ...
              <div className="p-8 space-y-6 bg-white dark:bg-slate-900">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input 
                      id="year" 
                      type="number" 
                      value={deptForm.established_year}
                      onChange={(e) => setDeptForm({...deptForm, established_year: e.target.value})}
                      placeholder="2024" 
                      className="h-12 rounded-xl bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 font-bold italic" 
                    />
                  </div>
                ...
                <DialogFooter className="pt-4">
                  <Button 
                    ...
                    className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-white font-black uppercase text-xs tracking-widest shadow-xl rounded-xl transition-all active:scale-95 disabled:opacity-50"
                  >
                    {isSubmitting ? "COMMITTING..." : "Commit Registry Record"}
                  </Button>
                </DialogFooter>
              </div>
            </form>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-32 flex flex-col items-center justify-center text-center px-4 bg-slate-50/20 dark:bg-transparent transition-all">
              ...
              <Button onClick={() => setIsSubmitOpen(true)} className="mt-8 bg-blue-600 hover:bg-blue-700 text-white font-black uppercase text-xs tracking-widest h-12 px-8 rounded-xl shadow-xl transition-all active:scale-95">Initiate Onboarding</Button>
            </div>
          ) : (
            <>
                        ...
                        <TableCell className="pl-8">
                          <div className="flex items-center gap-4">
                            <div className="w-11 h-11 rounded-2xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 flex items-center justify-center font-black text-xs border border-blue-100 dark:border-blue-900/30 shadow-sm transition-transform group-hover:scale-110">
                              {dept.name.split(' ').map((w: string) => w[0]).join('')}
                            </div>
                            <div>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-500 border border-slate-200 dark:border-slate-700">
                              {dept.hod?.split(' ').pop()?.charAt(0) || 'H'}
                            </div>
                            <span className="font-bold text-sm text-slate-700 dark:text-slate-300">{dept.hod || 'Unassigned'}</span>
                          </div>
                        </TableCell>
                        <TableCell className="text-center font-mono font-black text-xs text-slate-500 tabular-nums uppercase tracking-widest">{dept.established_year || 'N/A'}</TableCell>
                        <TableCell>
                          <div className="flex flex-col items-center gap-2">
                            <div className="flex items-center gap-4">
                              <div className="flex flex-col items-center">
                                <span className="flex items-center gap-1.5 text-xs font-black text-slate-900 dark:text-white tabular-nums">
                                  <Users size={12} className="text-blue-500" /> {dept.students || 0}
                                </span>
                              ...
                              <div className="flex flex-col items-center">
                                <span className="flex items-center gap-1.5 text-xs font-black text-slate-900 dark:text-white tabular-nums">
                                  <UserSquare2 size={12} className="text-indigo-500" /> {dept.tutors || 0}
                                </span>
